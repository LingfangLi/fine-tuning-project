...
def get_scores(model: HookedTransformer, graph: Graph, dataset, metric: Callable[[Tensor], Tensor]):
    ...
    for clean, corrupted, label in tqdm(dataset):
        ...
        with model.hooks(fwd_hooks=fwd_hooks_corrupted):
                corrupted_tokens=model.tokenizer(corrupted, padding="max_length", truncation=True, max_length=64, return_tensors="pt")["input_ids"][0]
                corrupted_logits = model(corrupted_tokens)

        with model.hooks(fwd_hooks=fwd_hooks_clean, bwd_hooks=bwd_hooks):
            clean_tokens=model.tokenizer(clean, padding="max_length", truncation=True, max_length=64, return_tensors="pt")["input_ids"][0]
            logits = model(clean_tokens)
            tokenized_label=model.tokenizer(label, padding="max_length", truncation=True, max_length=64, return_tensors="pt")["input_ids"][0]
            metric_value = metric(logits,tokenized_label)
